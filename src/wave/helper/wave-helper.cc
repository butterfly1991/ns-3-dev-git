...
WaveHelper::Install (const WifiPhyHelper &phyHelper, const WifiMacHelper &macHelper, NodeContainer c) const
{
  ...
  if (!isQosWaveMac)
    {
      NS_FATAL_ERROR ("macHelper should be QosWaveMacHelper or its subclass,"
                      "but QosWifiMacHelper is not allowed even QosWaveMacHelper"
                      "inherits from QosWifiMacHelper");
    }

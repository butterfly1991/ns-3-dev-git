                                                ...
                                                bool guardi, bool synci, bool switchi)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (m_coordinator->IsCchiAfter (duration), cchi, "now is " << now  << "ms "
                         "check whether is CCH interval after duration = " << after << "ms");
  NS_TEST_EXPECT_MSG_EQ (m_coordinator->IsSchiAfter (duration), schi, "now is " << now  << "ms "
                         "check whether is SCH interval after duration = " << after << "ms");
  NS_TEST_EXPECT_MSG_EQ (m_coordinator->IsGuardiAfter (duration), guardi, "now is " << now  << "ms "
...
ChannelCoordinationTestCase::DoRun ()
{
  ...
  // check channel coordination events
  Simulator::Schedule (Seconds (0.11), &ChannelCoordinator::Start, m_coordinator);
  Simulator::Schedule (Seconds (50.11), &ChannelCoordinator::Stop, m_coordinator);
  ...
  // check channel coordination events again with different CCHI and SCHI
  Simulator::Schedule (Seconds (0.11), &ChannelCoordinator::Start, m_coordinator);
  Simulator::Schedule (Seconds (50.11), &ChannelCoordinator::Stop, m_coordinator);
  Simulator::Stop (Seconds (51.0));
  Simulator::Run ();
...
AlternatingAccessTestCase::Receive (Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t mode, const Address &sender)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (coordinator->IsGuardiNow (), false, "we cannot receive any packet at guard interval");
  m_receives++;
...
AlternatingAccessTestCase::ReceiveInCch (Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t mode, const Address &sender)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (coordinator->IsSchiNow (), false, "we cannot receive any packet at SCH interval");
  m_cchReceives++;
...
AlternatingAccessTestCase::ReceiveInSch (Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t mode, const Address &sender)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (coordinator->IsCchiNow (), false, "we cannot receive any packet at CCH interval");
